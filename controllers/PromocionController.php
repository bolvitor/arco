...
class PromocionController
{
    ...
    public static function buscarAPI()
    {
        ...
        $promocionOficial = $_GET['per_promocion'];
        $arma = $_GET['per_arma'];
        ...
        AND mper.per_promocion = '$promocionOficial'
        AND mper.per_situacion = 11";

        if (!empty($arma)) {
            // Agregar la condición si la variable $arma no está vacía
            $sql .= " AND mper.per_arma = '$arma'";
        }

        $sql .= " ORDER BY per_grado DESC";
        ...
        foreach ($usuarios as $usuario) {
            $grado = $usuario['per_grado'];
            $catalogoOficial = $usuario['per_catalogo'];


            // Agrega los grados y catálogos al array
            $gradosCatalogos[] = [
                'grado' => $grado,
        ...
        foreach ($gradosCatalogos as $item) {
            $grado = $item['grado'];
            $catalogoOficial = $item['catalogo'];
            ...
            foreach ($meritos as $resultado) {
                $puntos_netos = $resultado['puntos_netos'];
                $catalogo = $resultado['per_catalogo'];


                $meritos1[] = [
                    'puntos_netos' => $puntos_netos,
                    'catalogo' => $catalogo,
                ];
            }
        }
        ...
        foreach ($gradosCatalogos as $item) {
            ...
            AND e_grado = '$grado'
            AND e_fecha = (SELECT MAX(e_fecha) FROM evaluaciones WHERE e_numero = 1 AND e_catalogo =  '$catalogoOficial' AND e_grado = '$grado')
            )ON mper.per_catalogo = e_catalogo
            INNER JOIN grados ON mper.per_grado = grados.gra_codigo
            ...
            foreach ($perfilBio as $resultado) {
                $catalogo = $resultado['per_catalogo'];
                $perfil_biofisico = $resultado['perfil_biofisico'];


                $perfilBio1[] = [
                    'perfil_biofisico' => $perfil_biofisico,
                    'catalogo' => $catalogo,
                ];
            }
        }
        ...
        foreach ($gradosCatalogos as $item) {
            ...
            foreach ($demeritos as $resultado) {
                $catalogo = $resultado['per_catalogo'];
                $punteo_demeritos = $resultado['punteo_demeritos'];


                // Agrega los grados y catálogos al array
                $demeritos1[] = [
                    'punteo_demeritos' => $punteo_demeritos,
                    'catalogo' => $catalogo,
                ];
            }
        }
        ...
        foreach ($gradosCatalogos as $item) {
            ...
            NOTAS_Z,
            CASE
            WHEN NOTAS_Z = 0 THEN 0 -- Establecer resultado_final en 0 cuando notas_r1 es 0
            ...
            $resultadoConsulta = Usuario::fetchArray($pafeSQL);

            foreach ($resultadoConsulta as $resultado) {
                $grado = $resultado['per_grado'];
                $catalogo = $resultado['per_catalogo'];
                $totalNotas = $resultado['suma_total'];
        ...
        foreach ($gradosYCatalogos as $item) {
            $grado = $item['grado'];
            $catalogoOficial = $item['catalogo'];
            ...
            $desempenio = "SELECT per_catalogo, per_grado, resultado_final
            from mper
            LEFT JOIN(
            SELECT
            sub.eva_cat1,
            ...
            $resultadodesempenio = Usuario::fetchArray($desempenio);

            foreach ($resultadodesempenio as $resultado) {
                $catalogo = $resultado['per_catalogo'];
                $resultado_final = $resultado['resultado_final'];


                // Agrega los resultados de esta  al array de resultados
                $resultadoeva[] = [
        ...
        try {
            ...
            $responseData = [
                ...
                'pafeSQL' => $resultadopafe,
                'desempenio' => $resultadoeva,
            ];
    ...
    public static function buscarOficial()
    {
        ...
        SUM(
            CASE
                ...
                WHEN LOWER(cond.con_desc_lg) LIKE '%placa de combatiente%' THEN 0.5
                WHEN LOWER(cond.con_desc_lg) LIKE '%roble de oro%' THEN 0.5
                WHEN LOWER(cond.con_desc_lg) LIKE '%distintivo%' THEN 1
                ELSE 0
            END
        ) AS meritos
        FROM dcon
